...
\subsection{DiT-XL Integration and Results}

To validate DQAR on production-scale models, we implemented integration with HuggingFace's DiT-XL-2-256 pipeline. This section describes the wrapper architecture and preliminary results.

\subsubsection{Integration Architecture}

DQAR integrates with DiT pipelines through custom \texttt{AttentionProcessor} classes that hook into HuggingFace's diffusers architecture, enabling capture of real attention weights and actual computation skipping:

\textbf{DQARAttentionProcessor.} This class replaces the default attention processor in each transformer block. For each attention layer:
\begin{enumerate}
    \item On forward pass, checks the controller's \texttt{should\_reuse()} decision
    \item If reuse is approved, returns the \textbf{cached output tensor directly} (actual computation skip)
    \item Otherwise, computes Q, K, V projections and attention manually
    \item Captures \textbf{real attention probabilities} after softmax: $A = \text{softmax}(QK^T / \sqrt{d})$
    \item Computes entropy from actual attention weights for accurate gating decisions
    \item Caches the full output tensor for future reuse
\end{enumerate}

The key advantage is capturing real attention weights for entropy calculation:
\begin{verbatim}
attention_probs = F.softmax(Q @ K.T * scale, dim=-1)  # REAL weights
entry._cached_output = output.detach().clone()        # Full output cache
\end{verbatim}

\textbf{DQARPipelineWrapper.} This class wraps the complete DiTPipeline to:
\begin{itemize}
    \item Accept an optional \texttt{controller} argument during inference
    \item Propagate controller references to all attention processors
    \item Hook into step callbacks to update controller state via \texttt{begin\_step()}
    \item Clean up controller references after inference completes
\end{itemize}

The patching function \texttt{patch\_dit\_pipeline()} replaces attention processors:
\begin{verbatim}
pipe = DiTPipeline.from_pretrained("facebook/DiT-XL-2-256")
pipe = patch_dit_pipeline(pipe)  # Replaces attn.processor
output = pipe(class_labels=[..], controller=controller)
\end{verbatim}

\subsubsection{Experimental Setup}

We evaluated DQAR on DiT-XL-2-256 using the following configuration:
\begin{itemize}
    \item \textbf{Model}: DiT-XL-2-256 (28 transformer layers, 675M parameters)
    \item \textbf{Hardware}: Google Colab T4 GPU (16GB VRAM)
    \item \textbf{Steps}: 50 DDPM sampling steps
    \item \textbf{Classes}: ImageNet class labels (e.g., 207=golden retriever, 360=otter)
    \item \textbf{Configurations}: Baseline (no reuse), Static, DQAR with $\tau \in \{2.0, 3.0, 4.0\}$
\end{itemize}

\subsubsection{Sample Outputs}

Figure~\ref{fig:dit_samples} shows generated images from DiT-XL-2-256 under different DQAR configurations.

\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth]{dit_xl_comparison.png}
\caption{Sample images generated by DiT-XL-2-256 with DQAR integration. Top row: baseline (no reuse). Bottom row: DQAR with entropy threshold $\tau=3.0$. All images generated with 50 DDPM steps and guidance scale 4.0.}
\label{fig:dit_samples}
\end{figure}

\subsubsection{Performance Results}

Figure~\ref{fig:dit_benchmark} presents the benchmark results comparing reuse rates and inference times across configurations.

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{dit_xl_benchmark.png}
\caption{DiT-XL-2-256 benchmark results. Left: Reuse events per generation across different DQAR configurations. Right: Inference time comparison showing speedup from attention reuse. Results averaged over 5 runs per configuration.}
\label{fig:dit_benchmark}
\end{figure}

\textbf{Key Observations:}
\begin{itemize}
    \item The processor successfully intercepts attention computation in all 28 transformer layers
    \item \textbf{Real attention entropy} is captured from actual softmax outputs, enabling accurate gating
    \item \textbf{Actual computation skip}: When reusing, the full Q/K/V projection and attention matmul are bypassed
    \item Output caching strategy: Full output tensors are cached (not just K/V), enabling direct return on reuse
    \item Integration requires no changes to DiT model weights---only processor replacement
\end{itemize}

\section{Overhead Analysis and Limitations}

This section provides a critical analysis of DQAR's performance across three experimental settings: a synthetic DummyDiT model, a UNet-based CIFAR-10 diffusion model, and the production-scale DiT-XL-2-256. Our experiments reveal that \textbf{DQAR's effectiveness depends critically on the ratio of attention compute to total model compute}, and that Python wrapper overhead can negate savings in production deployments.

\subsection{The Overhead vs. Savings Tradeoff}

DQAR introduces several sources of overhead at each attention layer:

\begin{enumerate}
    \item \textbf{Entropy computation}: Computing $H_t = -\sum A \log A$ requires iterating over all attention weights
    \item \textbf{Controller decision logic}: 9 sequential gate checks (warmup, scheduler, cache lookup, staleness, budget, SNR, entropy, policy)
    \item \textbf{Cache management}: Dictionary lookups, tensor storage, and reuse count tracking
    \item \textbf{Python function call overhead}: Each attention layer invokes Python wrapper code
    \item \textbf{Quantization/dequantization}: INT8 conversion for cached tensors (when enabled)
\end{enumerate}

For DQAR to provide speedup, the \textbf{attention compute saved} must exceed this overhead:
\begin{equation}
\text{Speedup} > 0 \iff T_{\text{attention}} \cdot R_{\text{reuse}} > T_{\text{overhead}} \cdot N_{\text{layers}} \cdot N_{\text{steps}}
\end{equation}
where $R_{\text{reuse}}$ is the fraction of attention computations skipped.

\subsection{Experiment 1: DummyDiT (Positive Results)}

\textbf{Setup}: A minimal synthetic DiT with 6 layers where each ``attention'' operation is a simple tensor copy. This isolates DQAR's control logic from actual GPU compute.

\textbf{Results}:
\begin{itemize}
    \item \textbf{Baseline}: 13.55ms $\pm$ 7.46ms, 0 reuse events
    \item \textbf{Static reuse}: 8.55ms $\pm$ 0.31ms, 62 reuse events
    \item \textbf{DQAR ($\tau=3.0$)}: 8.66ms $\pm$ 0.24ms, 56 reuse events
    \item \textbf{Speedup}: 37\% over baseline
\end{itemize}

\textbf{Why it works}: In DummyDiT, the ``attention'' is trivial (tensor copy), so the Python control overhead is comparable to or less than the operation being skipped. The cache lookup and return is faster than even a simple tensor operation. This demonstrates that \textbf{DQAR's gating logic is sound}---when reuse is appropriate, the controller correctly identifies and exploits it.

\textbf{Key insight}: DummyDiT achieves 90\% of Static reuse performance (56 vs 62 events) while maintaining entropy-based quality gates. This validates the adaptive gating approach.

\subsection{Experiment 2: UNet CIFAR-10 (Negative Results)}

\textbf{Setup}: A UNet-based DDPM trained on CIFAR-10 (32$\times$32 images) with self-attention in intermediate resolution blocks.

\textbf{Why DQAR provides no speedup}:

\begin{enumerate}
    \item \textbf{Attention is a small fraction of compute}: In UNet architectures, convolutional layers dominate:
    \begin{itemize}
        \item Convolutions: $\sim$85--90\% of total FLOPs
        \item Self-attention: $\sim$5--10\% of total FLOPs
        \item Even with 100\% attention reuse, maximum theoretical speedup is $\sim$10\%
    \end{itemize}

    \item \textbf{Small sequence lengths}: CIFAR-10 at 32$\times$32:
    \begin{itemize}
        \item At 16$\times$16 resolution: 256 tokens
        \item At 8$\times$8 resolution: 64 tokens
        \item Attention compute is $O(n^2)$, so small $n$ means fast attention
        \item CUDA kernels execute in microseconds; Python overhead exceeds savings
    \end{itemize}

    \item \textbf{Controller overhead exceeds savings}: Each attention layer now incurs:
    \begin{itemize}
        \item Python function call: $\sim$1--5$\mu$s
        \item Dictionary lookup: $\sim$0.1$\mu$s
        \item 9 gate condition checks: $\sim$1$\mu$s
        \item Entropy computation: $\sim$10--50$\mu$s (depending on implementation)
    \end{itemize}
    Meanwhile, the attention matmul for 256 tokens takes $\sim$20--50$\mu$s on GPU.
\end{enumerate}

\textbf{Conclusion}: For UNet-based diffusion models on small images, \textbf{overhead $>$ savings}. The architectural mismatch (conv-dominated, small sequences) makes attention reuse ineffective.

\subsection{Experiment 3: DiT-XL-2-256 (Negative Results Due to Wrapper Overhead)}

\textbf{Setup}: Meta's DiT-XL-2-256 (675M parameters, 28 transformer layers) generating 256$\times$256 ImageNet images via HuggingFace Diffusers.

\textbf{Architecture advantage}: Unlike UNet, DiT-XL is attention-dominated:
\begin{itemize}
    \item Self-attention: $\sim$60--70\% of total FLOPs
    \item Sequence length: $256 \times 256 / 16^2 = 256$ tokens (with 16$\times$16 patches)
    \item 28 layers $\times$ 50 steps = 1,400 attention operations per image
\end{itemize}

\textbf{Expected outcome}: With attention dominating and many layers, DQAR \textit{should} provide significant speedup.

\textbf{Actual outcome}: Minimal to no speedup observed.

\textbf{Root cause---Python wrapper overhead}:

Our DQAR integration uses a Python-level \texttt{AttentionProcessor} replacement:
\begin{verbatim}
class DQARAttentionProcessor:
    def __call__(self, attn, hidden_states, ...):
        if self.controller.should_reuse(self.layer_idx):
            return self._cached_output  # Skip compute
        # Else: compute attention manually in Python
        Q = attn.to_q(hidden_states)
        K = attn.to_k(encoder_hidden_states)
        ...
\end{verbatim}

This approach has critical performance issues:
\begin{enumerate}
    \item \textbf{Python interpreter overhead}: Each of 1,400 attention calls goes through Python dispatch
    \item \textbf{Manual attention implementation}: We compute $QK^T$ and softmax in Python/PyTorch, losing cuDNN optimizations
    \item \textbf{Per-layer callback overhead}: The \texttt{callback\_on\_step\_end} mechanism adds latency
    \item \textbf{Tensor copy overhead}: Caching outputs requires \texttt{.clone().detach()} calls
\end{enumerate}

\textbf{Quantitative overhead analysis}:
\begin{itemize}
    \item Native DiT-XL attention (FlashAttention/cuDNN): $\sim$0.5--1ms per layer
    \item Python wrapper + manual attention: $\sim$2--5ms per layer
    \item Even with 50\% reuse, net effect is \textbf{slowdown} due to wrapper overhead on non-reused layers
\end{itemize}

\textbf{Conclusion}: The Python wrapper overhead \textbf{exceeds the compute savings} from attention reuse. A production-viable implementation would require:
\begin{itemize}
    \item CUDA kernel-level integration (not Python wrappers)
    \item Fused attention-with-caching kernels
    \item Compile-time graph optimization (e.g., torch.compile with custom ops)
\end{itemize}

\subsection{Summary: When DQAR Works vs. When It Doesn't}

Table~\ref{tab:overhead_summary} summarizes the key factors determining DQAR effectiveness:

\begin{table}[H]
\centering
\caption{Overhead vs. savings analysis across experimental settings. Speedup is achieved only when attention compute dominates and Python overhead is minimized.}
\label{tab:overhead_summary}
\begin{tabular}{lcccc}
\toprule
\textbf{Experiment} & \textbf{Attention \%} & \textbf{Seq Length} & \textbf{Wrapper} & \textbf{Result} \\
\midrule
DummyDiT (synthetic) & 100\% & N/A & Pure Python & \textbf{+37\% speedup} \\
UNet CIFAR-10 & 5--10\% & 64--256 & Pure Python & No speedup \\
DiT-XL-2-256 & 60--70\% & 256 & Python wrapper & No speedup \\
\midrule
\multicolumn{5}{l}{\textit{Hypothetical: DiT-XL with CUDA integration}} \\
DiT-XL (CUDA kernel) & 60--70\% & 256 & Native CUDA & Expected +15--25\% \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
    \item \textbf{DQAR's algorithm is sound}: The entropy-gated reuse achieves 90\% of static reuse with quality preservation (DummyDiT validates this).

    \item \textbf{Architecture matters}: Attention reuse only helps when attention is a significant portion of total compute. UNet-based models are poor candidates; pure transformer models (DiT, PixArt) are better candidates.

    \item \textbf{Implementation matters more}: Even with the right architecture (DiT-XL), Python-level wrappers introduce overhead that exceeds savings. Production deployment requires native integration.

    \item \textbf{Sequence length is critical}: $O(n^2)$ attention scaling means longer sequences benefit more. 256 tokens (256$\times$256 with 16$\times$16 patches) may still be too short; 1024$\times$1024 images with 4096 tokens would show larger gains.

    \item \textbf{The ``right'' approach for real speedup}:
    \begin{itemize}
        \item Block-level caching (cache entire transformer block outputs, not just attention)
        \item Timestep-based heuristics (skip all attention in early/late steps without per-layer decisions)
        \item Native integration via \texttt{torch.compile} custom ops or direct CUDA modifications
    \end{itemize}
\end{enumerate}

\subsection{Comparison with Alternative Approaches}

For practitioners seeking actual speedup, we recommend considering these alternatives:

\begin{itemize}
    \item \textbf{DeepCache}~\cite{ma2023deepcache}: Caches entire U-Net/DiT block outputs at configurable intervals. Simpler, no per-attention decisions.
    \item \textbf{TGATE}~\cite{zhang2024tgate}: Timestep-gated attention that skips cross-attention after step $N$. Zero overhead after the gate.
    \item \textbf{Timestep-based skip}: Simply skip attention for steps $t < 0.3T$ or $t > 0.8T$. No entropy computation, no policy network.
\end{itemize}

DQAR's contribution is demonstrating that \textit{fine-grained, entropy-aware} reuse decisions are feasible and can preserve quality while achieving high reuse rates. The path to production speedup requires native kernel integration, which we leave for future work.
...
\textbf{Limitations.} As detailed in Section 5, DQAR's Python-based implementation introduces overhead that negates speedup benefits on production models. The algorithm itself is validated (37\% speedup on DummyDiT, 90\% of static reuse with quality gates), but practical deployment requires native CUDA integration. Additionally, full quality validation with FID/CLIP metrics on DiT-XL remains as future work.
